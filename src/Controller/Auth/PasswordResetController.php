...
final class PasswordResetController extends BaseController implements AuthController
{
    ...
    public function passwordReset(PasswordResetService $service, Request $request): Response
    {
...
//            if (!$service->sendResetPasswordLink($request)) {
//                return $this->redirectToRoute('auth_messages');
//            }
    ...
    public function passwordResetConfirm(ResettingRepository $repository, Request $request, string $token): Response
    {
        ...
        if (!$user) {
            // Token not found.
            $this->addFlash('warning', 'message.token.processed');
